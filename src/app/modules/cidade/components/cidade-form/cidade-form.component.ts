...
export class CidadeFormComponent implements OnInit, OnDestroy {
  ...
  handleSubmitEditCidade() {
    ...
    const requestEditCidade: Cidade = {
      cidId: this.cidadeAction.event.id as number,
      nome: this.editCidadeForm.value.nome as string,
      estado: {
        id: this.estadoSelecionado.id as number,
    ...
    if (this.cidadeAction && this.cidadeAction.cidadeList) {
      ...
      if (allCitys.length > 0) {
        const cityFiltered = allCitys.find(
          (element) => element?.cidId === cid_id
        );
        if (cityFiltered) {
          this.cidadesSelectedDatas = cityFiltered;

          this.estadoSelecionado = cityFiltered.estado as Estado;
          this.editCidadeForm.patchValue({
            nome: cityFiltered.nome,
            estado: {
              id: cityFiltered?.estado?.id,
            },
          });
        }
      }
