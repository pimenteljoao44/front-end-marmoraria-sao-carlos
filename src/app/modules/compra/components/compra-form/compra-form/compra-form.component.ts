...
export class CompraFormComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.addCompraForm = this.formBuilder.group({
      ...
      formaPagamento: [0, Validators.min(1)],
      observacoes: [''],
    });
  ...
  loadAllFormasPagamento(): void {
    this.allFormasPagamento = [
      { cod: 0, nome: 'Dinheiro' },
      { cod: 1, nome: 'Pix' },
      { cod: 2, nome: 'Cartão de Crédito' },
      { cod: 3, nome: 'Cartão de Débito' },
      { cod: 4, nome: 'Boleto Bancário' },
      { cod: 5, nome: 'Transferência Bancária' },
      { cod: 6, nome: 'Cheque' },
    ];
  }
  ...
  handleSubmitUpdateCompra(): void {
    ...
    const compraData = {
      ...
      formaPagamento: this.editCompraForm.value.formaPagamento.cod,
  ...
  handleAddItem(): void {
    ...
    const form = this.compraAction.event.action === 'Fazer uma compra'
      ? this.addCompraForm
      : this.editCompraForm;
  ...
  handleSubmit(): void {
    ...
    const compraData = {
      ...
      formaPagamento: this.addCompraForm.value.formaPagamento.cod,
      itensCompra: this.items.map((item) => ({
        quantidade: item.quantidade.toString(),
        valor: item.valor.toFixed(2),
        produto: item.produto.id,
